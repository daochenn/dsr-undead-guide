...
console.log('\nStep 2: Extracting ores and keys from consumables...');

// Создаем новые категории
jsonData.ore_items = [];
jsonData.key_items = [];

// Извлекаем ores и keys из consumable_items
...
// Показываем примеры
console.log('\n=== Sample Ore Items ===');
orderedData.ore_items.slice(0, 5).forEach(item => {
  console.log(`  - ${item.Name} (${item.Category})`);
});

console.log('\n=== Sample Key Items ===');
orderedData.key_items.slice(0, 5).forEach(item => {
  console.log(`  - ${item.Name} (${item.Category})`);
});
